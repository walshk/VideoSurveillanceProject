...
try:
	...
	while(True):
	    ...
	    if counter_pov == 0:
	    	top_left = gray[0:40][0:40]
	    	bottom_left = gray[0:40][-40:]
	    	top_right = gray[-40:][0:40]
	    	bottom_right = gray[-40:][-40:]
	    	if len(corners['top_left']) > 0:
	    		...
	    		avg = sum([tl_diff, tr_diff, bl_diff, br_diff])/4
	    		print(max([tl_diff, tr_diff, bl_diff, br_diff]))
